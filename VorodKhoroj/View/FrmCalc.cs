...
public partial class FrmCalc : Form
{
    ...
    private List<DateTime> overtimeinHoliday = [];
    ...
    private void DataGridConfig()
    {
        try
        {
            // اجرای شرط ها و سرویس از بیرون باید بیاد ، درستش کن فردا

            userid_txtbox.Text = _userid;

            var tempserv = new AttendanceCalculationService(_service);

            var collection = tempserv.Calculate(_userid, _fromDateTime, _toDateTime);

            dataView_calender.DataSource = collection;

            //  CalculatorData();
            DataGridViewConfig();
            Part2_Load();
            Labels = tempserv.Labels;
        }
        catch (Exception ex)
        {
    ...
    private void dataView_calender_RowPrePaint(object sender, DataGridViewRowPrePaintEventArgs e)
    {
        ...
        if (dataView_calender.Rows[e.RowIndex].Cells["IsNaghes"].Value is true)
            dataView_calender.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.Orange;

        if (overtimeinHoliday.Contains(DateTime.Parse(dataView_calender?.Rows[e.RowIndex]?.Cells["Date"]?.Value?.ToString())))
            dataView_calender.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.CadetBlue;
    }
