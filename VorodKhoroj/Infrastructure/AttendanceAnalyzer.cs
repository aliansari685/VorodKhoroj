...
public class AttendanceAnalyzer(AppServices appServices)
{
    ...
    // مقدار پیش‌فرض برای زمان تاخیر
    private TimeSpan _lateTm = TimeSpan.Parse("08:30:00");

    // ساعات کاری کامل برای انواع شرایط مختلف
    ...
    // ساعات کاری ناقص برای انواع شرایط مختلف
    ...
    // لیست روزهای غیبت، تعطیلات، رمضان و اضافه‌کاری در تعطیلات
    public List<DateTime> AbsenceDaysList { get; private set; } = [];
    public List<DateTime> HolidaysDaysList { get; private set; } = [];
    public List<EventDay> RamadanDaysList { get; private set; } = [];
    public List<DateTime> OverTimeHolidayList { get; private set; } = [];

    //Fluent Method:
    public AttendanceAnalyzer WithLateTime(TimeSpan lateTime)
    {
        _lateTm = lateTime; return this;
    }

    public AttendanceAnalyzer WithFullWorkTime(TimeSpan fullWorkTime)
    {
        _fullWorkTm = fullWorkTime; return this;
    }

    public AttendanceAnalyzer WithFullWorkThursdayTime(TimeSpan thursdayTime)
    {
        _fullWorkThursdayTm = thursdayTime; return this;
    }

    public AttendanceAnalyzer WithFullWorkFarvardinTime(TimeSpan farvardinTime)
    {
        _fullWorkFarvardinTm = farvardinTime; return this;
    }

    public AttendanceAnalyzer WithFullWorkRamadanTime(TimeSpan ramadanTime)
    {
        _fullWorkRamadanTm = ramadanTime; return this;
    }

    public AttendanceAnalyzer WithinCompleteWorkTime(TimeSpan fullWorkTime)
    {
        _incompleteWorkTm = fullWorkTime; return this;
    }

    public AttendanceAnalyzer WithinCompleteWorkThursdayTime(TimeSpan thursdayTime)
    {
        _incompleteWorkThursdayTm = thursdayTime; return this;
    }

    public AttendanceAnalyzer WithinCompleteWorkFarvardinTime(TimeSpan farvardinTime)
    {
        _incompleteWorkFarvardinTm = farvardinTime; return this;
    }

    public AttendanceAnalyzer WithinCompleteWorkRamadanTime(TimeSpan ramadanTime)
    {
        _incompleteWorkRamadanTm = ramadanTime; return this;
    }

    /// <summary>
    ///     محاسبه گزارش حضور و غیاب یک کاربر در بازه مشخصی از زمان.
    /// </summary>
    /// <param name="userId">شناسه کاربر</param>
    /// <param name="fromDateTime">تاریخ شروع</param>
    /// <param name="toDateTime">تاریخ پایان</param>
    /// <param name="autoEditIncompleteDays">آیا اصلاح خودکار روزهای ناقص فعال باشد؟</param>
    /// <returns>لیستی از رکوردهای کاری محاسبه شده</returns>
    public List<WorkRecord> Calculate(string userId, string fromDateTime, string toDateTime,
        bool autoEditIncompleteDays = true)
    {
        ...
        var dataFiltered = filtered.Select(x => new { x.UserId, x.DateTime }).OrderBy(x => x.DateTime);
        ...
        // تبدیل داده‌ها به WorkRecord و محاسبه اطلاعات هر روز
        WorkRecord[] groupedData = dataFilteredGrouped.Select(g =>
        {
            ...
            // اصلاح زمان خروج اگر فقط یک زمان ثبت شده باشد
            ...
            if (isThursday) exitInCompleteWorkTm = _incompleteWorkThursdayTm;
            if (isRamadan) exitInCompleteWorkTm = _incompleteWorkRamadanTm;
            if (isFarvardin) exitInCompleteWorkTm = _incompleteWorkFarvardinTm;
            if (maxDateTime == DateTime.MinValue)
                maxDateTime = autoEditIncompleteDays ? minDateTime.Date + exitInCompleteWorkTm : minDateTime;

            // اگر زمان ورودی و خروجی دوم وجود داشته باشد
            DateTime? minDateTime2 = orderedTimes.Count() > 2 ? orderedTimes.ElementAt(2) : null;
            DateTime? maxDateTime2 = orderedTimes.Count() > 2 && orderedTimes.Last() != minDateTime2
                ? orderedTimes.Last()
                : null;
            ...
            var standardWorkTime = isRamadan ? _fullWorkRamadanTm :
        ...
        var workDays = Enumerable.Range(0, (DateTime.Parse(toDateTime) - DateTime.Parse(fromDateTime)).Days + 1)
            .Select(i => DateTime.Parse(fromDateTime).AddDays(i)).ToList();
        ...
        Report = new AttendanceReport
        {
            ...
            TotalFractionTime =
                $@"{(int)fractionTime.TotalHours:D2}:{fractionTime.Minutes:D2}:{fractionTime.Seconds:D2}",
